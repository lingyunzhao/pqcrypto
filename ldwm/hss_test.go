// Copyright 2017 Lingyun Zhao. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package ldwm

import (
	"fmt"
	"io/ioutil"
	"testing"
)

func TestHssKeyGeneration(t *testing.T) {
	// otsTypecodes := []uint{LMOTSSHA256N32W1, LMOTSSHA256N32W2, LMOTSSHA256N32W4, LMOTSSHA256N32W8}
	// lmsTypecodes := []uint{LMSSHA256M32H5, LMSSHA256M32H10, LMSSHA256M32H15, LMSSHA256M32H20, LMSSHA256M32H25}
	otsTypecodes := []uint{LMOTS_SHA256_N32_W1}
	lmsTypecodes := []uint{LMS_SHA256_M32_H5}
	Ls := []int{3}
	for _, otsTypecode := range otsTypecodes {
		for _, lmsTypecode := range lmsTypecodes {
			for _, L := range Ls {

				hssPriv, privErr := GenerateHssPrivateKey(lmsTypecode, otsTypecode, L)
				if privErr != nil {
					t.Errorf("failed to generate a private key when lmsTypecode = %d, otsTypecode = %d, L = %d", lmsTypecode, otsTypecode, L)
				}
				hssPub := hssPriv.Public()

				phssPriv, pPrivErr := ParseHssPrivateKey(hssPriv.String())
				if pPrivErr != nil {
					t.Errorf("failed to parse a private key when lmsTypecode = %d, otsTypecode = %d, L = %d", lmsTypecode, otsTypecode, L)
				}

				phssPub, pPubErr := ParseHssPublicKey(hssPub.String())
				if pPubErr != nil {
					t.Errorf("failed to parse a public key when lmsTypecode = %d, otsTypecode = %d, L = %d", lmsTypecode, otsTypecode, L)
				}

				if hssPriv.String() != phssPriv.String() {
					fmt.Println(hssPriv.String())
					fmt.Println(phssPriv.String())
					t.Errorf("parsed HSS private != HSS private key when lmsTypecode = %d, otsTypecode = %d, L = %d", lmsTypecode, otsTypecode, L)
				}

				if hssPub.String() != phssPub.String() {
					t.Errorf("parsed HSS public != HSS public key when lmsTypecode = %d, otsTypecode = %d, L = %d", lmsTypecode, otsTypecode, L)
				}
			}
		}
	}
}

func TestHssSignandVerify(t *testing.T) {
	// otsTypecodes := []uint{LMOTSSHA256N32W1, LMOTSSHA256N32W2, LMOTSSHA256N32W4, LMOTSSHA256N32W8}
	// lmsTypecodes := []uint{LMSSHA256M32H5, LMSSHA256M32H10, LMSSHA256M32H15, LMSSHA256M32H20, LMSSHA256M32H25}
	otsTypecodes := []uint{LMOTS_SHA256_N32_W1}
	lmsTypecodes := []uint{LMS_SHA256_M32_H5}
	Ls := []int{3}
	for _, otsTypecode := range otsTypecodes {
		for _, lmsTypecode := range lmsTypecodes {
			for _, L := range Ls {
				hssPriv, _ := GenerateHssPrivateKey(lmsTypecode, otsTypecode, L)
				hssPub := hssPriv.Public()
				files, _ := ioutil.ReadDir("testdata")
				for i := 0; i < 10; i++ {
					for _, fi := range files {
						message, _ := ioutil.ReadFile(fi.Name())
						hssSig, hssSigErr := hssPriv.Sign(message)
						if hssSigErr != nil {
							t.Errorf("hssSig error lmsTypecode = %d, otsTypecode = %d, L = %d, file = %s", lmsTypecode, otsTypecode, L, fi.Name())
						}
						verifyErr := hssPub.Verify(message, hssSig)
						if verifyErr != nil {
							t.Errorf("verify error lmsTypecode = %d, otsTypecode = %d, L = %d, file = %s", lmsTypecode, otsTypecode, L, fi.Name())
						}
					}
				}
			}
		}
	}
}

func testVector1(t *testing.T) {
	message := []byte{
		,
	}

	signature := []byte{
		,
	}

	pubKey := new(HssPublicKey)
	pubKey.layer = 2
	pubKey.lmsPub = new(LmsPublicKey)
	pubKey.lmsPub.lmsTypecode = LMS_SHA256_M32_H5
	pubKey.lmsPub.otsTypecode = LMOTS_SHA256_N32_W8
	pubKey.lmsPub.id = []byte{0x61, 0xa5, 0xd5, 0x7d,
		0x37, 0xf5, 0xe4, 0x6b, 0xfb, 0x75, 0x20, 0x80,
		0x6b, 0x07, 0xa1, 0xb8}
	pubKey.lmsPub.t1 = []byte{0x50, 0x65, 0x0e, 0x3b,
		0x31, 0xfe, 0x4a, 0x77, 0x3e, 0xa2, 0x9a, 0x07,
		0xf0, 0x9c, 0xf2, 0xea, 0x30, 0xe5, 0x79, 0xf0,
		0xdf, 0x58, 0xef, 0x8e, 0x29, 0x8d, 0xa0, 0x43,
		0x4c, 0xb2, 0xb8, 0x78}

	verifyErr := pubKey.Verify(message, signature)
	if verifyErr != nil {
		t.Errorf("Test vector 1 failed")
	}
}

func testVector2(t *testing.T) {
	message := []byte{
		,
	}

	signature := []byte{
		,
	}

	pubKey := new(HssPublicKey)
	pubKey.layer = 2
	pubKey.lmsPub = new(LmsPublicKey)
	pubKey.lmsPub.lmsTypecode = LMS_SHA256_M32_H10
	pubKey.lmsPub.otsTypecode = LMOTS_SHA256_N32_W4
	pubKey.lmsPub.id = []byte{0xd0, 0x8f, 0xab, 0xd4,
		0xa2, 0x09, 0x1f, 0xf0, 0xa8, 0xcb, 0x4e, 0xd8,
		0x34, 0xe7, 0x45, 0x34}
	pubKey.lmsPub.t1 = []byte{0x32, 0xa5, 0x88, 0x85,
		0xcd, 0x9b, 0xa0, 0x43, 0x12, 0x35, 0x46, 0x6b,
		0xff, 0x96, 0x51, 0xc6, 0xc9, 0x21, 0x24, 0x40,
		0x4d, 0x45, 0xfa, 0x53, 0xcf, 0x16, 0x1c, 0x28,
		0xf1, 0xad, 0x5a, 0x8e}

	verifyErr := pubKey.Verify(message, signature)
	if verifyErr != nil {
		t.Errorf("Test vector 2 failed: %v", verifyErr)
	}

}

func TestRfc8554TestCases(t *testing.T) {
	testVector1(t)
	testVector2(t)
}
